...
const optionSchema = Yup.object()
  .shape({
    ...
    format: Yup.mixed().when({
      ...
      then: Yup.array().of(
        Yup.string()
          .required(unexpectedlyMissingField)
          .oneOf(AVAILABLE_OUTPUT_FORMATS),
      ),
    ...
    cjsMode: Yup.string()
      .required(unexpectedlyMissingField)
      .oneOf(AVAILABLE_CJS_MODES),
    declaration: Yup.string()
      .required(unexpectedlyMissingField)
      .oneOf(AVAILABLE_DECLARATION_MODES),
...
export const verifyConfig = async (
  ...
  try {
    ...
    const verifiedConfig = {
      ...config,
      outdir,
      format: Array.isArray(config.format)
        ? uniq(config.format)
        : [config.format],
