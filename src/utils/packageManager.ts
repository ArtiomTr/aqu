...
const checkPackageManager = async (name: string): Promise<PackageManager | undefined> => {
	try {
...
export const findAllInstalledManagers = async (): Promise<Array<PackageManager>> => {
...
export const getPackageVersion = async (pkg: string, manager: string) => {
	if (manager === 'npm' || manager === 'pnpm') {
		...
		const { stdout } = await execa(manager, ['info', pkg, 'version', '--silent']);
...
export const pickPackageManager = async (): Promise<string> => {
	...
	const out = await inquirer.prompt({
		...
		type: 'list',
		name: 'manager',
		choices: availableManagers.map((manager) => `${manager.name} ${chalk.gray(`(${manager.version})`)}`),
	});
