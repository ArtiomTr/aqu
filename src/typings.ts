...
export type VerifiedAquOptions = Omit<Required<AquOptions>, 'input' | 'format' | 'dtsBundleGeneratorOptions'> & {
	input: string[];
	format: Format[];
	dtsBundleGeneratorOptions?: DtsBundleGeneratorOptions;
};
...
export type TemplateScript = {
	initialize: (packageManager: string) => Promise<TemplateInitializationOptions>;
};
