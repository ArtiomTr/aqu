...
export const copyTemplate = async (
  ...
  const isDirs = await Promise.all(
    templateFiles.map((file) =>
      stat(join(from, file)).then((fileStats) => fileStats.isDirectory()),
    ),
  );
    ...
    templateFiles.map(async (templateFile, index) => {
      ...
      if (isDirs[index]) {
        await copyTemplate(
          ...
          join(to, templateFile),
          options,
          args,
        );
      } else if (
        existingFiles.includes(templateFile) &&
        filesToMergePaths.includes(resolvedPath)
      ) {
        const file = await mergeFiles(
          join(to, templateFile),
          join(from, templateFile),
        );
        ...
        await writeFile(
          join(to, templateFile),
          insertArgs(contents.toString(), args),
        );
      } else {
