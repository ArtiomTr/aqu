...
import {
  creationOptionInvalid,
  creationOptionNotSpecfied,
...
export const verifyCreateOptions = async (
  ...
  const createOptionsSchema = Yup.object().shape({
    ...
    license: Yup.string()
      .required(creationOptionNotSpecfied)
      .oneOf(availableLicenses, creationOptionInvalid),
    template: Yup.string()
      .required(creationOptionNotSpecfied)
      .oneOf(availableTemplates, creationOptionInvalid),
  });
