...
const { copy, pathExists, readdir, remove } = (fsExtra as unknown as { default: typeof fsExtra }).default;
...
export const buildOptions: AquSelfBuildOptions = {
	...
	esbuild: {
		...
		external: ['jest-watch-typeahead/testname', 'jest-watch-typeahead/filename', 'esbuild'],
	},
};
...
export const buildTemplates = () =>
	new Promise<void>((resolve, reject) => {
		rimraf(buildOptions.templates.outdir, async () => {
			try {
				await copy(buildOptions.templates.sourceDir, buildOptions.templates.outdir);

				const paths = (await readdir(buildOptions.templates.sourceDir)).map((pth) =>
					join(buildOptions.templates.sourceDir, pth, buildOptions.templates.templateScriptFilename),
				);
				...
				await Promise.all(
					entryPoints.map(async (entry) => {
						...
						const dir = join(buildOptions.templates.outdir, dirname(relative(buildOptions.templates.sourceDir, entry)));
						await build({
